...
public interface TbSimCardCopyDao {
    ...
    @Update("update tbSimCardCopy set Status=#{1},MdfBy=#{2},MdfTm=NOW() where IdxIccid=#{0}")
    int updateSimCard(String idxIccid, int status, String MdfBy);

    @Update("update tbSimCardCopy set restNetData=#{1},Status=#{2},MdfBy=#{3},MdfTm=NOW() where IdxIccid=#{0}")
    int updateSimCardData(String idxIccid,double restNetData,int status,String MdfBy);

    @Update("update tbSimCardCopy set restNetData=#{1},MdfBy=#{2},MdfTm=NOW() where IdxIccid=#{0}")
    int updateSimCardData2(String idxIccid,double restNetData,String MdfBy);

    @Update("update tbSimCardCopy set restNetData=#{0} where status=0 or status=1")
    void updateNetData(int restNetData);

    @Delete("delete from tbSimCardCopy where idxIccid = #{0}")
