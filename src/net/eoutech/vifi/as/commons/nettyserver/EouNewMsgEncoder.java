 ...
 */
public class EouNewMsgEncoder extends MessageToByteEncoder<EouData> {
    @Override
    protected void encode(ChannelHandlerContext ctx, EouData eouData, ByteBuf byteBuf) throws Exception {
        ...
        try {
            ...
            LogUtils.info("BUFFER:"+buffer+"---Channel---"+ctx.channel().toString()+"---SERVER Encode response message OK, length is " + buffer.limit());
        }catch (Exception e){
            e.printStackTrace();
